...
public class Lava3 : MonoBehaviour
{
    ...
    public Vector2 uvAnimationRate1 = new Vector2( 0.0f, -1.0f );
    public string textureName1 = "_MainTex";
	public Vector2 uvAnimationRate2 = new Vector2( 0.0f, -1.0f );
    public string textureName2 = "_BlendTex";
	
	public Vector2 m_UVOffsetA;
	public Vector2 m_UVOffsetB;
	public Vector2 temp_UVOffsetA = new Vector2(0.0f, 0.0f);
	public Vector2 temp_UVOffsetB = new Vector2( 0.0f, 0.0f );
	...
	public LavaManager m_WaterManager = null;
	...
	public void Start() 
	{
		...
		m_IsDirty = false;
		m_WaterManager = LavaManager.Instance();
	}
		...
		if(mode >= WaterMode.FastAndNoRefraction)
		{
			...
			if(IsUnderwater(cam)) 
			{
				...
				*/
				...
				if(Camera.main == cam && !cam.gameObject.GetComponent(typeof(Lava3UnderwaterEffect)) ) {
					cam.gameObject.AddComponent(typeof(Lava3UnderwaterEffect));	
				}
				
				Lava3UnderwaterEffect effect = (Lava3UnderwaterEffect)cam.gameObject.GetComponent(typeof(Lava3UnderwaterEffect));				
				if(effect) {
					effect.enabled = true;
					effect.m_Water = this;
				}
	...
	{	
		...
		Vector4 displacement4 = LavaManager.Instance().GetMaterialVector("_Displacement");
		Vector4 displacementXz = LavaManager.Instance().GetMaterialVector("_DisplacementXz");
		...
		float valX = Mathf.Sin(tiling * vertex.x + LavaManager.Instance().m_Timer * speed);
		float valY = Mathf.Sin(tiling2 * vertex.z + LavaManager.Instance().m_Timer  * speed2);
		...
		vertex.y += LavaManager.Instance().GetDisplaceMeshAmountAt(vertex, transform);
	...
	void Update()
	{
		...
		//if(Application.isPlaying) {
			m_IsDirty |= LavaManager.Instance().DisplaceMesh(GetMesh(), transform);
		//}					

	    // Scroll UV to make Lava flow
		// Change vector constacts to manipulate velocity of scrolling UVs
		temp_UVOffsetA += m_UVOffsetA * Time.deltaTime;
		temp_UVOffsetB += m_UVOffsetB * Time.deltaTime;
	
		// Change Texture Offset based on Velocity and Delta Time
        if( GetComponent<Renderer>().enabled )
        {
			//LavaManager.Instance().SetTextureOffset( textureName1, uvOffset1 );
			//LavaManager.Instance().SetTextureOffset( textureName2, uvOffset2 );
			
			GetComponent<Renderer>().sharedMaterial.SetTextureOffset( textureName1, temp_UVOffsetA  );
			GetComponent<Renderer>().sharedMaterial.SetTextureOffset( textureName2, temp_UVOffsetB  );
		}
